...
function LoadInitialQuibsQuibStreamforcarousel(){
    $('.quibsdata').append("<div class='quib-item movie-poster-wrapper' prevcol='PrevCol' style='text-align: center;'>" +
        "<div class='panel panel-default quib-item-inner inner1' style=' background-color: white; height:100%'>" +
        "<div class='panel-body' style='padding: 0 ;height:100%'>" +
        "<div style='font-size: 24px; font-weight: bold; color: black;padding:7% 0px'>Timeline quibs for</div>" +
        "<img src='" + $('.MoviePosterThumb').attr('src') + "' />" +
...
function GetInitialQuibsQuibStream() {
    ...
    $.ajax({
        async: false,
        url: localStorage.getItem('environment') + '/GetInitialQuibsByMovieId',
        dataType: 'text',
        data: { MovieId: localStorage.getItem('MovieId'), QuibZero: false, SelectStream: selectedStreams },
        success: function (response) {
            if (response != undefined && response != null) {
                content = response;
            }
...
function GetQuibsByMovieIdForInterval(startTime, endTime) {
    ...
    $.ajax({
        async: false,
        url: localStorage.getItem('environment') + '/GetQuibsByMovieIdForInterval',
        dataType: 'text',
        data: { MovieId: localStorage.getItem('MovieId'), QuibZero: false, SelectedStream: selectedStreams, StartTime: startTime },
        success: function (response) {
            if (response != undefined && response != null) {
                content = response;
            }
